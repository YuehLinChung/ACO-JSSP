...
def initialization():
    ...
    numMachines = 700
    parameterInitialization(20, 20) #input number of ants and cycles //Needed before creating node object

# SELECT weaving_machine_schedule.*, warping_beam_order.warp_beam_order_no
# FROM weaving_machine_schedule
# INNER JOIN warping_beam_order
# ON  warping_beam_order.manufacture_order_id = weaving_machine_schedule.manufacture_order
# WHERE start_datetime > '2020-09-15'
    DBconfig=['localhost', 'root', 'combo5210', 'wayson-dev-0901']
    date_after = '2020-09-01'
    conn = pymysql.connect(*DBconfig)
    scheduled_order = pd.read_sql(
"""SELECT weaving_machine_schedule.*, warping_beam_order.warp_beam_order_no, schedule_weaving.weaving_start_datetime,schedule_weaving.weaving_end_datetime
FROM weaving_machine_schedule
INNER JOIN warping_beam_order
ON  warping_beam_order.manufacture_order_id = weaving_machine_schedule.manufacture_order
INNER    JOIN schedule_weaving
ON schedule_weaving.manufacture_order_id = weaving_machine_schedule.manufacture_order
WHERE start_datetime > '%s'"""%date_after, conn)
    conn.close()
    scheduled_order = scheduled_order[['weaving_machine_core','manufacture_order','warp_beam_order_no','start_datetime','end_datetime','weaving_start_datetime','weaving_end_datetime']]
    # (scheduled_order['end_datetime']-scheduled_order['start_datetime']) #需要天數
    numMachines = len(scheduled_order.loc[:,'weaving_machine_core'].unique())
    global config
    config = ScheduleConfig()
    for it in scheduled_order.itertuples():
        duration = (it.end_datetime - it.start_datetime).days +1
        start_idx = (it.weaving_start_datetime - datetime.datetime.strptime(date_after, '%Y-%m-%d')).days#最快上機
        deadline_idx = (it.weaving_end_datetime - datetime.datetime.strptime(date_after, '%Y-%m-%d')).days#最晚了機
        if start_idx < 0:
            # deadline_idx -= start_idx
            start_idx = 0
        if deadline_idx - start_idx + 1 < duration:
            continue
        config.addJob([],duration,it.weaving_machine_core, orderNo=it.manufacture_order, clothNo=it.warp_beam_order_no, start=start_idx, deadline=deadline_idx)
# =============================================================================
...
# =============================================================================
    ...
    global machineList
    machineList = defaultdict(list)#[[] for i in range(numMachines)]
...
class Node:
    ...
    def __repr__(self):
        return 'Node(num:%d duration:%d machine:%d ready:%d deadline:%d start:%d end:%d)'%(self.num, self.duration, self.machine, self.readyTime, self.deadline, self.startTime, self.endTime)
...
class Ant:
    ...
    def __repr__(self):
        return 'Ant: {} Pos: {}'.format(self.num, self.position)
...
def chooseClique(antNum):
    machinesWithJob = [i for i in machineList if len(machineList[i])!=0]#range(numMachines)
...
# =============================================================================
    node.endTime = node.startTime + node.duration -1
    bestSoFarNODESLIST[node.num].endTime = node.startTime + node.duration -1
...
def findEarliestStartTime(node: Node, ant: Ant, machine):
    ...
    """
    # if machine == 114 and node.orderNo==187:
    #     breakpoint()
    ...
    if len(nodeList)==0 and len(blockedSchedule.get(machine))==0:
        return max([n.endTime for n in node.init_deps]+[node.readyTime], default=0)
    ...
    bucket = [False]*(node.duration + max([n[2]+1 for n in blockedSchedule.get(machine)]+[max_end_time,node.readyTime], default=0))
    bucket[:node.readyTime] = [True]*node.readyTime
...
def detectChain(deploy: list) -> bool:
    ...
    if len(deploy) < 2:
        return False
    _machineList = set(i['machine'] for i in deploy)
    for machine in _machineList:
...
def constructionPhase(): #The Probabilistic Construction Phrase of solutions begins by K ants
    for c in range(C):
        ...
        for ant in ANTLIST:
            skip_counter = 0
            machinesWithJob = [i for i in machineList if len(machineList[i])!=0]
                ...
                if c<1:
                    ...
                    for x in machineList[currentMachine]:
                        ...
                        if skip_counter%len(machineList[currentMachine]) != 0:
                            ...
                            if b < 0: #超出deadline
                                solutionGraphList[ant.num][oldAntPosition][ant.position] = -50*b
                                # additional--start
                    ...
                    for _ in range(len(machineList[currentMachine])):
                        ...
                        if skip_counter%len(machineList[currentMachine]) != 0:#numJobs
                            ...
                            if a < 0: #太早開始做
                                ...
                                #     T[i][moveFrom] *= (1.02**c)
                            if b < 0: #超出deadline
                                solutionGraphList[ant.num][moveFrom][moveTo] = -50*b #NODESLIST[moveTo].duration*5
                                ...
                                for s in NODESLIST:
                                    if solutionGraphList[ant.num][s.num][moveTo] > 0:
                                        solutionGraphList[ant.num][s.num][moveTo] *= (0.99**(-b))
                                    if solutionGraphList[ant.num][s.num][moveFrom] > 0:
                                        solutionGraphList[ant.num][s.num][moveFrom] /= (0.95**(-b))
                                # additional--end
                            else:
...
#FIXME: 計算endTime時 時間應包含開始那天 (start + duration -1)
        # node.endTime -= 1
        ...
        if node.deadline - node.readyTime + 1 < node.duration:
            status = 'Bad Deadline'
        elif node.startTime < node.readyTime:
            status = 'Too Early'
        elif node.endTime > node.deadline:
            status = 'Not Meet'
        elif node.endTime <= node.deadline:
            status = 'Meet'
        df.append(dict(Machine='m'+str(node.machine), Start=str(s), Finish=str(d),
                      Task=str(node.orderNo),#'{:02}'.format(node.orderNo),
    ...
    df.sort(key=lambda n:(n['Machine']), reverse=True)#n['Status'], 
    for block in blockedSchedule.block:
        ...
        df.append(dict(Machine=str(block[0]), Start=str(s), Finish=str(d),
                      ...
                      ReadyTime='Null',
                      ClothNo='Null'))
    fig = px.timeline(df, x_start="Start", x_end="Finish", y="Machine", color="Status", hover_data=["Duration","ClothNo","ReadyTime","Deadline"], text="Task", title='Schedule from %s'%totalStartTime, height=3000, category_orders={'Machine':[x['Machine'] for x in df]})
    fig.update_yaxes(autorange="reversed")
    with open('re_schedule.html', 'w') as f:
        f.write(fig.to_html())
        f.close()

#FIXME: 加上幅寬
def getMachineList(cloth_no, wide=True):
    conn = pymysql.connect('localhost', 'root', 'combo5210', 'wayson-dev-0901', connect_timeout=3)
    cursor = conn.cursor()
    cursor.execute("""SELECT cloth_tissue.fit_machine_type
FROM cloth_core
    INNER JOIN cloth_tissue
    ON cloth_core.cloth_tissue_id = cloth_tissue.cloth_tissue_id
WHERE cloth_core.cloth_no='%s'"""%cloth_no)
    fit_machine_type = cursor.fetchone()[0]
    cursor.close()
    del cursor

    #WHERE weaving_machine_core.axle_width = 1 AND machine_type=1
    SQL = """SELECT weaving_machine_core_id
FROM weaving_machine_core"""
    if wide or fit_machine_type==1:
        SQL += '\nWHERE'
        if wide:
            SQL += ' weaving_machine_core.axle_width = 1'
            if fit_machine_type==1:#只能用龍頭織
                SQL += ' AND machine_type=1'
        elif fit_machine_type==1:
            SQL += ' machine_type=1'
# SELECT weaving_machine_core_id
# FROM weaving_machine_core
# WHERE weaving_machine_core.axle_width = 1 AND machine_type=1

    data = pd.read_sql_query(SQL,conn)# AND weaving_machine_core.beam_width = 1
    conn.close()
...
def getSolutions(numOfSegs, taskLength, start_date, deadline, clothNo, DBconfig, wide=True):
    """
    ...
    length = taskLength
    try:
        machine_list = getMachineList(clothNo, wide)
    except:
        machine_list = []
    _start_date = datetime.datetime.strptime(start_date, '%Y-%m-%d')
...
def main(days:int, numSeg:int, start_date:str, deadline:str, clothNo:str, DBconfig:list, wide:bool=True) ->str:
    """
    API for JEP.
    ...
    # initialization()
    # constructionPhase()
    solutions = getSolutions(numOfSegs=numSeg, taskLength=days, start_date=start_date, deadline=deadline, clothNo='WS-1234', DBconfig=DBconfig, wide=wide)
    result = JsonString()
...
# start = time()
# DBconfig=['localhost', 'root', 'combo5210', 'wayson-dev-0901']
# solutions = getSolutions(numOfSegs=2, taskLength=20, start_date='2020-09-03', deadline='2020-10-03',
#                          clothNo='WS-1234',DBconfig=DBconfig,wide=False)
...
# # solutions = main(days=10, numSeg=2, start_date='2020-09-02', deadline='2020-10-03', clothNo='WS-1234', DBconfig=['localhost','root','combo5210','wayson-dev-0901'], wide=False)
